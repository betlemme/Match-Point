...
/**
 * Servlet che aggiorna rispettivamente il campo di gioco, la data e l'ora di una partita
 * 
 * @author GEMMED
 * @version 1.00
 */
@SuppressWarnings("serial")
public class UpdateCampoServlet extends AbstractDatabaseServlet {
	/**
	 * @param req
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			campo = Integer.parseInt(req.getParameter("campo"));
			
			//creo bean per aggiornare vincitore
			PartitaSingola partita = new PartitaSingola(numeroPartita, id);
			
			new UpdatePartitaSingolaDatabase(DS.getConnection(), partita).updateCampo(campo);
