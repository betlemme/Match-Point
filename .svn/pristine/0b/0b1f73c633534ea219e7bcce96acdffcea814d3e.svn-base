		...
		<div class="row">
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<ul>
							...
							<li>Città: <c:out value="${giocatore.citta} "/></li>
							<li>Provincia: <c:out value="${giocatore.provincia} "/></li>
			...
			<div class="col-md-9">
				...
				<ul class="nav nav-tabs">
					...
					<li><a href="#torneiattivi" data-toggle="tab">Tornei da disputare</a></li>
					<li><a href="#torneiincorso" data-toggle="tab">Tornei in corso</a></li>
				</ul>
				...
				<div class="tab-content">
					...
					<!-- Tab pane per tornei da disputare -->
					<div class="tab-pane fade" id="torneiattivi">
						<h1>Tornei Da Disputare</h1>
						<table class="table table-bordered table-striped">
							<thead>
								<tr>
									<th>Nome</th>
							...
							<tbody>
								<c:forEach var="torneo" items="${torneidadisputare}">
									<form method="GET" action="<c:url value="/profiloTorneo"/>">
										<input type="hidden" name="id" value="${torneo.id}">
										<tr>
											<td><button type="submit" class="btn btn-default">
									...
									<c:choose>
										<c:when test="${!torneo.isIscrizioneConclusa}">
											<td><a   onclick="setIDTorneoElim(${torneo.id})"
											data-toggle="modal" data-target="#eliminazione">Disicriviti</a></td>
									</c:when>
										<c:otherwise>
									<td><a ><del>Disicriviti</del></a></td>
										</c:otherwise>
									</c:choose>
					...
					<!-- Tab pane per tornei in corso -->
					<div class="tab-pane fade" id="torneiincorso">
						<h1>Tornei Attivi</h1>
						<table class="table table-bordered table-striped">
							<thead>
								<tr>
							...
							<tbody>
								<c:forEach var="torneo" items="${torneiincorso}">
									<form method="GET" action="<c:url value="/profiloTorneo"/>">
										<input type="hidden" name="id" value="${torneo.id}">
										<tr>
											<td><button type="submit" class="btn btn-default">
...
<!-- Modale modifica profilo -->
	...
	<div class="modal fade" id="eliminazione" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"
						aria-hidden="true">&times;</button>
					<h4 class="modal-title" id="myModalLabel">Sei sicuro di disicriverti?</h4>
				</div>
				<div class="modal-body">

					<form method="POST" action="<c:url value="/iscrizione-torneo"/>"
						class="form-horizontal" role="form" id="eliminaform">
...
<script>
	    function setIDTorneo(id) {
	    	document.getElementById('iscrizioneform').action = "<c:url value='/iscrizione-torneo?azione=crea&id='/>"+id;
	    };   
    
	    function setIDTorneoElim(id) {
	    	document.getElementById('eliminaform').action = "<c:url value='/iscrizione-torneo?azione=elimina&id='/>"+id;
	    }; 
