...
/**
 * Servlet che modifica l'ora di una partita.
 * 
 * @author GEMMED
 * @version 1.00
 */
@SuppressWarnings("serial")
public class UpdateTimeServlet extends AbstractDatabaseServlet {
	/**
	 * @param req
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			ora = Time.valueOf(req.getParameter("time")+ ":00");
			
			//creo bean per aggiornare vincitore
			PartitaSingola partita = new PartitaSingola(numeroPartita, id);
			
			new UpdatePartitaSingolaDatabase(DS.getConnection(), partita).updateOra(ora);
